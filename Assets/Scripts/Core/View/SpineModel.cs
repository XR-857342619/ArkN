...
public class SpineModel : UnitModel
{
    ...
    {
        this.Unit = unit;
        //if (SkeletonAnimation.name == unit.UnitData.Model)
        //Debug.Log(SkeletonAnimation.SkeletonDataAsset.name + "/" + unit.UnitData.Model);
        if (unit is Units.干员)
            //SkeletonAnimation.SkeletonDataAsset = SpineImportHelper.Instance.animations["test"];
    ...
    protected virtual void updateState()
    {
        ...
        if (Unit.Height > 0) SkeletonAnimation.transform.localPosition = new Vector3(0, Unit.Height/SkeletonAnimation.transform.lossyScale.y, Unit.Height * 0.45f);//没什么道理的z轴偏移
    ...
    protected virtual void changeAnimation(string[] animations)
    {
        ...
        var loop = (animations[0] == "Start" || animations[0] == "Appear" || animations[0] == "Die" || animations[0] == "Reborn_1_End") ? false : true;
